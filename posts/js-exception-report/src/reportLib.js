...
var BJ_REPORT = (function(global) {
    ...
    global.onerror = function(msg, url, line, col, error) {
        ...
        if (_isOBJByType(newMsg, 'Event')) {
            newMsg += newMsg.type
                ? '--' + newMsg.type + '--' + (newMsg.target ? newMsg.target.tagName + '::' + newMsg.target.src : '')
                : '';
        }
    ...
    var _submit = function(bl_param) {
        ...
        _bl_param = {
            id: id,
            ext: JSON.stringify(_config.ext)
        };
        for (var key in bl_param) {
            _bl_param[key] = bl_param[key];
        }
        var blParamString = JSON.stringify(_bl_param);

        if (isRepeat(blParamString)) {
            return;
        }
        var search = '?bl_param=' + encodeURIComponent(blParamString) + '&t=' + +new Date();
        url = _config.url + search;
    ...
    var _send = function(isReoprtNow) {
        while (_error.length) {
            ...
            if (_isOBJByType(_config.ignore, 'Array')) {
                for (var i = 0, l = _config.ignore.length; i < l; i++) {
                    ...
                    if (
                        (_isOBJByType(rule, 'RegExp') && rule.test(error_str[1])) ||
        ...
        if (count) {
            var comboReport = function() {
                ...
                _submit({error: error_list.join('&') + '&count=' + count});
                comboTimeout = 0;
                error_list = [];
            };
